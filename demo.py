...
BM_DICT = {"Affine1D": Affine1D, "Quadratic1D": Quadratic1D}
...
def run_experiment():
    ...
    ## flags, constants
    smoke_test = False 
    wandb_mode = "online"  # if not smoke_test else "disabled"
    plot = False
    max_plot_tasks = 5
    config = dict(
        seed_pyro=123,
    ...
    if wandb_mode != "disabled":
        wandb.login()
    with wandb.init(project="mtbnn_v0", config=config, mode=wandb_mode) as wandb_run:
        # wandb stuff
        ...
        x_pred_meta = np.linspace(-1.5, 1.5, config["n_points_pred"])[
            None, :, None
        ].repeat(config["n_tasks_meta"], axis=0)
        # test benchmark
        ...
        x_pred_test = np.linspace(-1.5, 1.5, config["n_points_pred"])[
            None, :, None
        ].repeat(config["n_tasks_test"], axis=0)
            ...
            if config["n_hidden"] == 0:
                ...
                with warnings.catch_warnings():
                    ...
                    fig = plot_distributions(
                        ...
                        samples_posterior_meta=samples_posterior_meta,
                        samples_posteriors_test=samples_posteriors_test,
                        n_contexts_test=config["n_contexts"],
                    )
...
def main():
    # set up sweep
    from mtbnn import _allowed_prior_types
    import math
    ...
    #     "method": "grid",
    #     "parameters": {
    #         "bm": {"values": ["Quadratic1D", "Affine1D"]},
    #         "prior_type": {"values": _allowed_prior_types + ["fixed"]},
    #     },
    # }
    sweep_config = {
        "name": "regularizer_alpha_Quadratic1D",
        "method": "random",
        "metric": {
            "name": "eval/marg_ll_target",
            "goal": "maximize",
        },
        "parameters": {
            "bm": {"value": "Quadratic1D"},
            "prior_type": {"value": "multivariate_normal"},
            "alpha_reg": {
    ...
    pprint(sweep_config)
    sweep_id = wandb.sweep(sweep_config, project="mtbnn_v0")
    # TODO: do proper multiprocessing
    wandb.agent(
        sweep_id=sweep_id,
